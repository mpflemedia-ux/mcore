(function () {
  ...
  async function seedPhion() {
    var ids = {};
    var keys = Object.keys(PHION);
    for (var i = 0; i < keys.length; i++) {
      status(t('Seeding ', 'Seed ') + keys[i]);
      ids[keys[i]] = await child(ROOT, keys[i]);
      var subs = PHION[keys[i]];
  ...
  async function packClient(id) {
    for (var i = 0; i < CLIENT_SLOTS.length; i++) await child(id, CLIENT_SLOTS[i]);
  }
  async function seedRoot() {
    try {
      await ensureToken();
      var ids = await seedPhion();
      var clients = await listClients(ids['05_Clients']);
      for (var i = 0; i < clients.length; i++) {
        if (clients[i].name.indexOf('_') === 0) continue;
        status(t('Client slots ', 'Slot client ') + (i + 1) + '/' + clients.length + ' — ' + clients[i].name);
        await packClient(clients[i].id);
      }
      status(t('SOP level-2 ready', 'SOP level-2 sedia') + ' — ' + clients.map(function (c) { return c.name; }).join(', '));
  ...
  async function seedClient() {
    ...
    try {
      await ensureToken();
      var ids = await seedPhion();
      var clientId = await child(ids['05_Clients'], name);
      await packClient(clientId);
  ...
  function inject() {
    ...
    wrap.innerHTML =
      '<button type="button" class="btn btn-outline btn-sm" id="docs-seed-root">' + t('Seed folders', 'Seed folder') + '</button>' +
      '<button type="button" class="btn btn-outline btn-sm" id="docs-seed-client">' + t('New client', 'Client baru') + '</button>' +
