...
self.addEventListener('fetch',e=>{
  ...
  if(isNav||/\/app\/?$/.test(url.pathname)||url.pathname.endsWith('/index.html')){
    e.respondWith(fetch(e.request,{cache:'no-store'}).then(async res=>{
      ...
      if(!html.includes('customer-public-form.js')) html=html.replace('</body>','<script src="./customer-public-form.js?v=1"></script></body>')
      else html=html.replace(/customer-public-form\.js\?v=\d+/g,'customer-public-form.js?v=1')
