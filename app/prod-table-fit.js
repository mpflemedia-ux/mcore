(function () {
  function fit() {
    ...
    wrap.style.maxHeight = 'calc(100dvh - ' + (300 + extra) + 'px)';
  ...
  function wrapFn() {
    var orig = window._invLoad;
    if (typeof orig !== 'function' || orig._prodFit) return;
    window._invLoad = async function () {
      var r = orig.apply(this, arguments);
      if (r && r.then) r.then(function () { setTimeout(fit, 0); setTimeout(fit, 80); });
      else { setTimeout(fit, 0); setTimeout(fit, 80); }
      return r;
    };
    window._invLoad._prodFit = true;
  }
  wrapFn();
