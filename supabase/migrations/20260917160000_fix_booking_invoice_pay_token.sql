-- Invoice for paid bookings: ref_no (not invoice_no), public_token, skip if price=0
...
BEGIN
  ...
  IF cust_id IS NULL THEN
    cust_code := 'BKCUS-' || upper(substr(md5(random()::text || clock_timestamp()::text), 1, 8));
  ...
  END IF;
  qref := 'BK-' || to_char(now() AT TIME ZONE 'Asia/Kuala_Lumpur', 'YYYYMMDD') || '-' || substr(md5(random()::text), 1, 6);
  ...
  v_status := 'hold';
  IF coalesce(svc.price, 0) > 0 THEN
    inv_tok := substr(md5(random()::text || clock_timestamp()::text || book_id::text), 1, 32);
    inv_ref := 'INV-BK-' || to_char(now() AT TIME ZONE 'Asia/Kuala_Lumpur', 'YYYYMMDD') || '-' || substr(md5(book_id::text), 1, 6);
    BEGIN
      INSERT INTO public.invoices (
        ...
        svc.price, 0, 'unpaid', 'Booking '||qref||' '||svc.name, inv_tok
      ) RETURNING id INTO inv_id;
    EXCEPTION WHEN OTHERS THEN
      BEGIN
        INSERT INTO public.invoices (
          tenant_id, customer_id, customer_name, ref_no, issue_date, due_date, total, status, public_token
        ) VALUES (
          tn.id, cust_id, trim(p_name), inv_ref, CURRENT_DATE, CURRENT_DATE, svc.price, 'unpaid', inv_tok
        ) RETURNING id INTO inv_id;
      EXCEPTION WHEN OTHERS THEN
    ...
    IF inv_id IS NOT NULL THEN
      BEGIN
        ...
        VALUES (tn.id, inv_id, svc.name||' ['||qref||']', 1, svc.price, svc.price);
      EXCEPTION WHEN OTHERS THEN
        BEGIN
          INSERT INTO public.invoice_items (invoice_id, description, qty, unit_price, line_total)
          VALUES (inv_id, svc.name||' ['||qref||']', 1, svc.price, svc.price);
        EXCEPTION WHEN OTHERS THEN
          NULL;
        END;
      END;
  ...
  RETURN jsonb_build_object(
    ...
    'amount', coalesce(svc.price, 0), 'invoice_id', inv_id, 'pay_token', inv_tok,
